...
# Script to deploy only the order service resources with Terraform using Docker
set -e
...
echo "🚀 Deploying order service resources with Terraform (Environment: ${ENVIRONMENT})"
...
# Check if the order service database already exists
DB_INSTANCE=$(aws rds describe-db-instances --query "DBInstances[?DBName=='orderdb'].DBInstanceIdentifier" --output text 2>/dev/null || echo "")
if [ "$DB_INSTANCE" != "None" ] && [ "$DB_INSTANCE" != "" ]; then
  echo "Found existing order service database: $DB_INSTANCE"
  echo "Importing order service database into Terraform state..."
  run_terraform import module.order_service_db.aws_db_instance.this $DB_INSTANCE || echo "Order service database import failed, but continuing..."
fi

# Create a plan file for the order service modules
echo "📋 Creating plan for order service modules..."
run_terraform plan \
  -var-file=${TERRAFORM_VARS_FILE} \
  -target=module.order_service_db \
  -target=module.eks_order_service \
  -out=order-service.tfplan || {
...
# Apply the plan
echo "🔧 Applying order service plan..."
run_terraform apply -auto-approve order-service.tfplan

echo "✅ Order service Terraform deployment completed."
echo "Now run the deploy-order-service.sh script to deploy the Kubernetes resources."